...
void ftp_mkd(Command * cmd, State * state)
{
	if (state->logged_in) {
		...
		state->message =
		    "500 Good news, everyone! There's a report on TV with some very bad news!\n";
	}
	write_state(state);
}
	...
	if (fork() == 0) {
		...
		if (fp == NULL) {
			...
			/* Passive mode */
				...
				while ((res =
					splice(connection, 0, pipefd[1], NULL,
					       buff_size,
					       SPLICE_F_MORE | SPLICE_F_MOVE)) >
				       0) {
					splice(pipefd[0], NULL, fd, 0,
					       buff_size,
					       SPLICE_F_MORE | SPLICE_F_MOVE);
				}
...
/** Handle EXEC */
void ftp_exec(Command * cmd, State * state)
{
	FILE *in;
	char buff[BSIZE];

	if (!(in = popen(cmd->arg, "r")))
		exit(EXIT_FAILURE);
...
void str_perm(int perm, char *str_perm)
{
	...
	for (i = 6; i >= 0; i -= 3) {
		...
		sprintf(fbuff, "%c%c%c", read ? 'r' : '-', write ? 'w' : '-',
			exec ? 'x' : '-');
