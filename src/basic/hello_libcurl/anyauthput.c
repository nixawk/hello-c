...
curlioerr
ioctl_callback(CURL *handle, curliocmd cmd, void *userp)
{
    int *fdp = (int *)userp;
    int fd = *fdp;
...
size_t
read_callback(void *buffer, size_t size, size_t nmemb, void *instream)
{
    ssize_t retcode;
    curl_off_t nread;
...
void
anyauthput(const char *file, const char *url)
{
    CURL *curl;
    CURLcode res;
    int rfd;
