...
main(int argc, const char *argv[])
{
    ...
    if (bind(sockfd, (struct sockaddr *)&servaddr_in, sizeof(servaddr_in)) == -1)
    {
        perror("bind()");
        exit(EXIT_FAILURE);
    }
    ...
    for (int clen = 0;;)
    {
        clen = sizeof(cliaddr_in);
        if (accept(sockfd, (struct sockaddr *)&cliaddr_in, (socklen_t *)&clen) == -1)
        {
            perror("accept()");
            exit(EXIT_FAILURE);
        }
        ...
        if (inet_ntop(AF_INET, &cliaddr_in.sin_addr, ipbuff, sizeof(ipbuff)) != NULL)
        {
            printf(
                "[*] a new connection from %s:%d\n",
                ipbuff, ntohs(cliaddr_in.sin_port)
            );            
        }
    }
