...
main(int argc, const char *argv[])
{
    ...
    for (;;)
    {
        ...
        nrecv = recvfrom(sfd, buf, BUF_SIZE, 0, (struct sockaddr *) &caddr, &structlen);
        if (nrecv == -1)
        {
            perror("recvfrom");
            exit(EXIT_FAILURE);
        ...
        printf("Server received %ld bytes from %s\n", (long) nrecv, caddr.sun_path);

        if (sendto(sfd, buf, nrecv, 0, (struct sockaddr *) &caddr, structlen) != nrecv)
        {
            perror("sendto");
            exit(EXIT_FAILURE);
        }
