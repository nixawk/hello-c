...
int main(int argc, const char *argv[])
{
	...
	strncpy(caddr.sun_path, "/tmp/unix_domain_client",
		sizeof(caddr.sun_path) - 1);
	...
	if (bind(sfd, (struct sockaddr *)&caddr, sizeof(struct sockaddr_un) - 1)
	    == -1) {
		perror("bind");
		exit(EXIT_FAILURE);
	}
	...
	if (sendto
	    (sfd, buf, buflen, 0, (struct sockaddr *)&saddr,
	     sizeof(struct sockaddr_un)) != buflen) {
		perror("sendto");
		exit(EXIT_FAILURE);
	}
